  ...
  app.get('/albumart', hasPermRead, function(req, resp, next) {
    ...
    fs.exists(albumArtFile, function(exists) {
      ...
      fetchImageFromFile(albumArtFile, file, function(exists, err) {
        ...
        fetchImageFromFolder(albumArtFile, file, function(exists) {
          if(exists) {
            serveAlbumCover(albumArtFile, resp);
            return;
          }
...
*/
function fetchImageFromFolder(albumArtFile, file, callback) {
 var self = this;
 ...
 fs.readdir(dir, function(err, files) {
   ...
   var file = fs.createWriteStream(albumArtFile);
   file.on("finish", function() {
     callback(true);
   });
